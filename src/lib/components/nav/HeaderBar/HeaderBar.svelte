<script lang="ts">
	...
	import esportsData from '$lib/store/esportsDataSlice';
	import { faBoxOpen, faChevronDown, faChevronRight } from '@fortawesome/free-solid-svg-icons';
	import { onDestroy } from 'svelte';
	import Fa from 'svelte-fa/src/fa.svelte';
	import AccountDropdown from '../Dropdown.svelte';
	...
	const setAccountToggle = () => {
		accountToggle = !accountToggle;
	};

	let cartToggle = false;

	const setCartToggle = (toggle: boolean) => {
		cartToggle = toggle;
	};
		...
		>
			...
			>
				...
				<li>
					<a href="/signup">
						<Button toggle>
							<div slot="text" class="py-0 uppercase text-inherit text-zinc-200">Sign up</div>
						</Button>
					</a>
				</li>
				<!-- <li>
          ...
          >
            ...
            <AccountDropdown show={accountToggle} />
				...
				<li>
					<div
						class="group relative block flex-1 space-x-1 rounded py-2 pr-4 pl-3 text-center text-inherit no-underline  dark:text-white md:p-0"
						on:click={() => setCartToggle(true)}
						on:mouseenter={() => setCartToggle(true)}
						on:mouseleave={() => setCartToggle(false)}
					>
						<Fa class="inline-block  self-center align-top" icon={faChevronDown} size="md" />
						<div class="relative inline-block">
							<Fa
								class="tranzinc-y-1/3 inline-block transform self-center align-top"
								icon={faBoxOpen}
								size="xs"
							/>
							<span
								class="-tranzinc-y-1/4 absolute right-0 -top-0 inline-block h-3 w-4 transform rounded-full border-white text-xs font-bold text-emerald-500 shadow-2xl duration-100"
							>
								{esportsData.getAllPrices($esportsData).length}
							</span>
						</div>
						<!-- <div class="col-span-2 inline-block content-end text-xl">
							<sup>&euro</sup>
							<div class="-ml-1 inline text-xl">
								{(esportsData.getTotalPrice($esportsData) ?? 0) / 100 ??
									'Coming Soon'}
							</div>
							<div class="-ml-2 inline text-sm lowercase">
								/{$esportsData.billingInterval}
							</div>
						</div> -->
						<CartDropdown show={cartToggle} />
					</div>
				</li>
			</ul>
		</div>
